...
from cjxl_sweep_common import (
    StudyError as SweepSummaryError,
    DEFAULT_QUALITIES,
    SCHEDULE_EFFORT_MAJOR,
    SCHEDULE_PHASE_MAJOR_SHUFFLED,
    PHASE_SEED_OFFSETS as PHASE_SEED_OFFSETS,
    quality_to_distance,
    all_tuple_jobs,
    phase_shuffle_seed,
    shuffled_tuple_jobs,
...
def tuple_summary(timing_records, stage_records):
    ...
    for identifier, samples in sorted(timings.items()):
        try:
            row = summarize_timing_samples(samples)
        except SweepSummaryError as error:
